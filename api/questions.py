...
def get_question(question_key: str, user = Depends(get_current_user),db = Depends(get_db)):
    return CRUDQuestion.get_question_by_key(question_key,db=db)

@router.put("/upvote/{question_key}")
def upvote_question(question_key: str, user = Depends(get_current_user),db = Depends(get_db)):
    return CRUDQuestion.upvote_question(question_key,db=db)

@router.put("/downvote/{question_key}")
def downvote_question(question_key: str, user = Depends(get_current_user),db = Depends(get_db)):
    return CRUDQuestion.downvote_question(question_key,db=db)

@router.put("/views/{question_key}")
def viewed_question(question_key: str, user = Depends(get_current_user),db = Depends(get_db)):
    return CRUDQuestion.viewed_question(question_key,db=db)
